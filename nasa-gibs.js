...
(() => {
  ...
  const layers = {
    ...
    clouds: { label: 'CLOUD FRACTION', layer: 'MODIS_Cloud_Fraction_Day', format: 'image/png', alpha: 0.60 },
    temp: { label: 'LAND TEMPERATURE', layer: 'MODIS_Terra_Land_Surface_Temp_Day', format: 'image/png', alpha: 0.60 },
    night: { label: 'NIGHT LIGHTS', layer: 'VIIRS_Black_Marble', format: 'image/jpeg', alpha: 0.76 },
    gpm: { label: 'GPM PRECIPITATION', layer: 'IMERG_Precipitation_Rate', format: 'image/png', alpha: 0.78 }
  };
  ...
  for (const [id, cfg] of Object.entries(layers)) {
    ...
    button.className = 'gibs-btn' + (id === 'gpm' ? ' gpm' : '') + (id === activeId ? ' on' : '');
  ...
  function wmsUrl(cfg, date) {
    const p = new URLSearchParams({ service:'WMS', version:'1.1.1', request:'GetMap', layers:cfg.layer, styles:'', srs:'EPSG:4326', bbox:'-180,-90,180,90', width:'2048', height:'1024', format:cfg.format, transparent:cfg.format === 'image/png' ? 'true' : 'false', time:date });
    return `${WMS}?${p.toString()}`;
  }

  async function buildLayer(id, date) {
    const cfg=layers[id];
    const provider=await Cesium.SingleTileImageryProvider.fromUrl(wmsUrl(cfg,date),{rectangle:WORLD,credit:`NASA GIBS · ${cfg.layer}`});
    return new Cesium.ImageryLayer(provider,{alpha:cfg.alpha,show:true});
  }

  async function setLayer(id) {
    const cfg=layers[id]; if(!cfg) return false;
    for(const daysAgo of [0,1,2,3]){
      ...
      try{
        ...
        activeLayer=viewer.imageryLayers.add(layer,0); activeId=id; activeDate=date;
        panel.querySelector('#gibsLayer').textContent=cfg.label; panel.querySelector('#gibsDate').textContent=date;
        grid.querySelectorAll('.gibs-btn').forEach(b=>b.classList.toggle('on',b.dataset.layer===id));
        window.NASAGIBS={activeId,date,layer:activeLayer,setLayer}; return true;
      }catch(error){console.warn(`GIBS ${id} unavailable for ${date}`,error)}
    }
    panel.querySelector('#gibsDate').textContent='UNAVAILABLE'; return false;
  }
