...
(async () => {
  ...
  scene.globe.atmosphereLightIntensity=6.2; scene.globe.lambertDiffuseMultiplier=1.06;
  ...
  scene.skyAtmosphere.atmosphereLightIntensity=23.0; scene.skyAtmosphere.mieCoefficient=16e-6;
  scene.skyAtmosphere.mieAnisotropy=0.76; scene.fog.enabled=false; scene.highDynamicRange=true;
  (async()=>{try{
    ...
    const layer=new Cesium.ImageryLayer(provider); layer.brightness=1.06; layer.contrast=1.05; layer.saturation=1.10;
  ...
  const orbitEntity=viewer.entities.add({id:'ISS-ORBIT-PATH',name:'ISS ±50 minute orbit path',polyline:{positions:[],width:2,material:new Cesium.PolylineGlowMaterialProperty({glowPower:.18,taperPower:.4,color:Cesium.Color.CYAN.withAlpha(.48)}),clampToGround:false,arcType:Cesium.ArcType.NONE}});
  const groundTrack=viewer.entities.add({id:'ISS-GROUND-TRACK',name:'ISS ground track',polyline:{positions:[],width:1.5,material:Cesium.Color.CYAN.withAlpha(.20),clampToGround:true,arcType:Cesium.ArcType.GEODESIC}});
  let autoOrbit=true,followISS=false,heading=5.08,lastManual=0,phase=0;
  ...
  window.toggleEarthFollow=()=>{followISS=!followISS;if(followISS)autoOrbit=false;else viewer.camera.lookAt(Cesium.Cartesian3.ZERO,new Cesium.HeadingPitchRange(heading,-.16,11800000));return followISS};
  window.resetEarth=()=>{autoOrbit=true;followISS=false;heading=5.08;phase=0;viewer.camera.lookAt(Cesium.Cartesian3.ZERO,new Cesium.HeadingPitchRange(heading,-.16,11800000));document.getElementById('auto')?.classList.add('on');document.getElementById('follow')?.classList.remove('on')};
  window.updateISS3D=(lat,lon,altKm)=>{const p=Cesium.Cartesian3.fromDegrees(lon,lat,Math.max(altKm||0,0)*1000);iss.position=p;halo.position=p;if(followISS)viewer.camera.lookAt(p,new Cesium.HeadingPitchRange(0,-.10,1250000));};
  window.updateOrbitPath=(sat,date)=>{if(!sat||!window.satellite)return;const op=[],gp=[];for(let m=-55;m<=55;m++){const t=new Date(date.getTime()+m*60000),pv=window.satellite.propagate(sat,t);if(!pv?.position)continue;const geo=window.satellite.eciToGeodetic(pv.position,window.satellite.gstime(t));const lat=window.satellite.degreesLat(geo.latitude),lon=window.satellite.degreesLong(geo.longitude),alt=Math.max(geo.height,0)*1000;op.push(Cesium.Cartesian3.fromDegrees(lon,lat,alt+18000));gp.push(Cesium.Cartesian3.fromDegrees(lon,lat,0))}orbitEntity.polyline.positions=op;groundTrack.polyline.positions=gp};
  viewer.camera.lookAt(Cesium.Cartesian3.ZERO,new Cesium.HeadingPitchRange(heading,-.16,11800000));
  scene.postRender.addEventListener(()=>{const now=performance.now();if(autoOrbit&&!followISS&&now-lastManual>250){phase+=.00052;heading+=.000022;const pitch=-.16+Math.sin(phase*.45)*.022;const range=11800000+Math.sin(phase*.31)*450000;viewer.camera.lookAt(Cesium.Cartesian3.ZERO,new Cesium.HeadingPitchRange(heading,pitch,range));}});
